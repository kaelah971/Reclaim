...
/// @title ProtectedPaymentEscrowV2
/// @notice Strict state-machine escrow with opt-in permissionless autopilot release.
///         V1 semantics are preserved exactly for manual payments (autopilotEnabled = false).
///         Autopilot payments (autopilotEnabled = true) may be released by ANY caller via
///         executeAutoRelease once deliveryAt + max(autoReleaseSeconds, disputeWindowSeconds)
///         has passed. Manual (false) is default-safe.
contract ProtectedPaymentEscrowV2 is ReentrancyGuard, Ownable {
    ...
    enum State {
        ...
        ReleaseRequested, // 4 — Worker requested funds release (manual only)
    ...
    struct Payment {
        ...
        uint64 acceptedAt;
        uint64 deliveryAt;
        // Packed group C: 2 × uint64 + bool (8 + 8 + 1 = 17B ≤ 32B)
    ...
    /// @param autopilotEnabled    True to enable permissionless auto-release via
    ///                            executeAutoRelease; false keeps the V1 manual flow.
    ///                            Included in the termsHash. Manual (false) is default-safe.
    ...
    function createPayment(
        ...
        p.autopilotEnabled = autopilotEnabled;
    ...
    ///      Each re-submission overwrites deliveryAt and therefore RESTARTS the
    ///      autopilot countdown (eligibleAt = deliveryAt + max(auto, dispute)).
    ...
    /// @notice Worker requests release of funds after delivery (manual payments only).
    /// @dev Does NOT transfer funds — client must still approve.
    ///      Autopilot payments cannot use this step; they release via executeAutoRelease.
    /// @param paymentId The payment.
    function requestRelease(uint256 paymentId) external onlyWhenNotPaused {
        Payment storage p = _payments[paymentId];
        if (p.paymentId == 0) revert PaymentNotFound();
        if (p.autopilotEnabled) revert InvalidState();
        if (msg.sender != p.worker) revert UnauthorizedWorker();
    ...
    /// @dev Uses checks-effects-interactions + nonReentrant. Works for both manual
    ///      and autopilot payments (manual client override is always preserved).
    ...
    /// @notice Permissionless auto-release for autopilot payments once the wait elapses.
    /// @dev ANY caller (client, worker, stranger EOA, or cron executor) may execute once
    ///      eligible — identity is irrelevant and no role check is performed.
    ///      Only DeliverySubmitted is eligible; ReleaseRequested is NOT eligible (it is
    ///      only reachable for manual payments). A Disputed payment reverts via the state
    ///      check. Re-submitting evidence overwrites deliveryAt and RESTARTS the countdown.
    ///      eligibleAt = deliveryAt + max(autoReleaseSeconds, disputeWindowSeconds),
    ///      computed with uint256 math (uint64 inputs — no overflow risk in practice).
    ///      Emits both PaymentReleased and AutoReleased.
    /// @param paymentId The autopilot payment to release.
    function executeAutoRelease(uint256 paymentId) external nonReentrant onlyWhenNotPaused {
        Payment storage p = _payments[paymentId];
        if (p.paymentId == 0) revert PaymentNotFound();
        if (p.state != State.DeliverySubmitted) revert InvalidState();
        if (!p.autopilotEnabled) revert InvalidState();
        if (p.evidenceReference == bytes32(0)) revert EvidenceRequired();
        if (p.autoReleaseSeconds == 0) revert InvalidState();

        uint256 waitSeconds = uint256(p.autoReleaseSeconds);
        if (uint256(p.disputeWindowSeconds) > waitSeconds) {
            waitSeconds = uint256(p.disputeWindowSeconds);
        }
        uint256 eligibleAt = uint256(p.deliveryAt) + waitSeconds;
        if (block.timestamp < eligibleAt) revert InvalidState();
        if (escrowToken.balanceOf(address(this)) < p.amount) revert TransferAmountMismatch();

        // Effects — cache everything before the external call (CEI)
        uint256 releaseAmount = p.amount;
        address workerAddr = p.worker;
        address clientAddr = p.client;
        uint64 deliveryAt = p.deliveryAt;
        uint64 eligibleAt64 = uint64(eligibleAt);

        p.state = State.Released;
        p.releasedAt = uint64(block.timestamp);

        // Interaction — single transfer of exactly the escrowed amount
        escrowToken.safeTransfer(workerAddr, releaseAmount);

        emit PaymentReleased(paymentId, clientAddr, workerAddr, releaseAmount);
        emit AutoReleased(paymentId, clientAddr, workerAddr, releaseAmount, msg.sender, deliveryAt, eligibleAt64);
    }
    ...
    ///      Funded / Accepted / ReleaseRequested behave exactly as V1.
    ///      For DeliverySubmitted, disputes are only allowed within
    ///      deliveryAt + disputeWindowSeconds. A zero disputeWindowSeconds means there
    ///      is NO post-delivery dispute window: disputes on DeliverySubmitted revert.
    /// @param paymentId       The payment.
    ...
    function openDispute(uint256 paymentId, bytes32 disputeReference) external onlyWhenNotPaused {
        ...
        // Post-delivery dispute window gate (V1 had no time gate here).
        if (p.state == State.DeliverySubmitted) {
            if (p.disputeWindowSeconds == 0) revert InvalidState();
            if (block.timestamp > uint256(p.deliveryAt) + uint256(p.disputeWindowSeconds)) {
                revert InvalidState();
            }
        }
    ...
    /// @notice Read-only auto-release eligibility for off-chain executors/cron.
    /// @dev Pure state read — performs no transfers and changes no state.
    /// @param paymentId The payment ID.
    /// @return eligible True when executeAutoRelease would succeed (ignoring escrow balance).
    /// @return eligibleAt deliveryAt + max(autoReleaseSeconds, disputeWindowSeconds), or 0
    ///         when it cannot be computed yet.
    /// @return reason One of "ok", "wrong_state", "not_autopilot", "no_evidence",
    ///         "no_reference_yet", "timers_disabled", "too_early".
    function getAutoReleaseEligibility(uint256 paymentId)
        external
        view
        returns (bool eligible, uint64 eligibleAt, string memory reason)
    {
        Payment storage p = _payments[paymentId];
        if (p.paymentId == 0) revert PaymentNotFound();
        if (p.state != State.DeliverySubmitted) return (false, 0, "wrong_state");
        if (!p.autopilotEnabled) return (false, 0, "not_autopilot");
        if (p.evidenceReference == bytes32(0)) return (false, 0, "no_evidence");
        if (p.deliveryAt == 0) return (false, 0, "no_reference_yet");
        if (p.autoReleaseSeconds == 0) return (false, 0, "timers_disabled");

        uint256 waitSeconds = uint256(p.autoReleaseSeconds);
        if (uint256(p.disputeWindowSeconds) > waitSeconds) {
            waitSeconds = uint256(p.disputeWindowSeconds);
        }
        uint64 computedAt = uint64(uint256(p.deliveryAt) + waitSeconds);
        if (block.timestamp < uint256(computedAt)) return (false, computedAt, "too_early");
        return (true, computedAt, "ok");
    }
